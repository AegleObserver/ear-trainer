...
底部「演奏」Tab（`PlayPage`）为**网格多轨编辑器 + 播放器**，规格主文档平行存放于 `play/PROMPT.md`（本文件仅保留要点速览与交叉引用）。

要点速览（详见 `play/PROMPT.md`）：

- **网格 UI**：纵轴 = 音高轴、横轴 = 时间轴；步进式（固定格点）放置，音符时值 = 最小分度整数倍
- **状态栏**（网格正上方）：拍号 4/4 / 3/4（切换清空网格可撤回）+ BPM 数字输入/预设档位（60–240，默认 120，tab 内独立）+ 最小分度 1/8/1/16 + 撤回/恢复（仅音符编辑）+ 播放/暂停/停止
- **编辑交互**：空白格点击 + 水平拖拽建音符（点击已占格清除整音符，新建音符即时发声预览）；切换最小分度仅影响后续吸附，已放音符不动
- **多音轨**（网格左侧书签）：切换/新增/删除（≥1）+ 每轨独立音色（复用节奏测试音色池：鼓 C1 + 5 档乐音 A4，按 voice 缓存独立实例）+ 静音开关 + 色块点击试听音色（A4）
- **播放**：从 `start_point`（点击网格顶部小节设置，红色游标）起播，至旋律结束或手动停止；基于 `Tone.Transport` 调度，支持暂停/继续；播放期间锁定编辑
- **手稿管理**（右侧手稿栏）：保存/加载/重命名/删除创作，localStorage 持久化（上限 10，常量可调）
- **响应式布局**：三栏（音轨 | 网格 | 手稿）；网格四小节 `ResizeObserver` 等比放缩占满中间；网格可用宽低于默认宽时手稿栏移至音轨正下方（左侧堆叠），侧栏永不遮挡网格
- **导出**（规划项）：MP3/WAV 导出现阶段空置
- **编辑记忆**：切 Tab 保留编辑——沿用「常驻挂载、hidden 显隐」约定（`AppShell`），无额外持久化
