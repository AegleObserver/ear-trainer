...
- **网格等比放缩**：`PitchGrid` 用 `ResizeObserver` 测量容器宽，格宽 `cellW = (容器宽 − 音高标签列 40px) / 总步数`——**四个小节始终占满中间 GridPane**，音符 / 格线 / 小节头 / 起点游标 / 拖拽预览 / 点击判定全部随 `cellW` 同步缩放（横向成比例，行高不变）
- **手稿栏回流**：`PlayPage` 测量布局行宽，当**网格可用宽低于默认宽**（总步数 × 16px，即三栏并排会压缩网格）时，手稿栏从右侧移至**音轨正下方**（左侧堆叠），让出更多宽度给网格
- **永不遮挡网格**：音轨 / 手稿栏在任意宽度下始终位于左侧或左右两侧，不会出现在网格上方
